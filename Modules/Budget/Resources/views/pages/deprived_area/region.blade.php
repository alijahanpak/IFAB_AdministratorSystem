<div class="tabs-panel table-mrg-btm dynamic-height-level1" id="region">
    <div id="dynamicParentId1">
    @if(\Modules\Budget\Entities\DeprivedArea::isExistRegion())
                ...
                @foreach($dAreas as $dArea)
                    @if($dArea->daCoId != '' && $dArea->daReId != '' && $dArea->daRdId == '' && $dArea->daViId == '')
                        <div class="grid-x {{ $rowColor % 2 == 1 ? 'tableRowColor' : '' }} selectAbleRow">
                            <div class="medium-3 table-contain-border cell-vertical-center">{{ \Modules\Admin\Entities\Region::find($dArea->daReId)->reName }}</div>
                            <div class="medium-3 table-contain-border cell-vertical-center">{{ \Modules\Admin\Entities\County::find($dArea->daCoId)->coName }}</div>
                            <div class="medium-6 table-contain-border cell-vertical-center">
                                <div class="grid-x">
                                    ...
                                    <div class="medium-1 cell-vertical-center text-left">
                                        <a class="dropdown small sm-btn-align display-off"  type="button" data-toggle="daActionDropdown_region{{ $dArea->id }}"><img width="15px" height="15px"  src="{{ asset('pic/menu.svg') }}"></a>
                                        <div class="dropdown-pane dropdown-pane-sm " data-close-on-click="true"  data-hover="true" data-hover-pane="true"  data-position="bottom" data-alignment="right" id="daActionDropdown_region{{ $dArea->id }}" data-dropdown data-auto-focus="true">
